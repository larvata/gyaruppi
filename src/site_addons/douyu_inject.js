...
var updateBellBtnVisibility = (subscribed) => {
  if (subscribed) {
  ...
  if (type === 'updateSubscribeState') {
    ...
    if (init) {
      ...
      postLoadingInterval = setInterval(() => {
        ...
        var roomTitleContainer = document.querySelector(roomTitleSelector);
        currentRoomTitle = roomTitleContainer.innerText;
        roomTitleContainer.before(subscribeIcon);
        ...
        updateBellBtnVisibility(subscribed);
      }, 1000);
    }

    // not init load, update the icon by state
    !postLoadingInterval && updateBellBtnVisibility(subscribed);
  }
});
