...
export default (room) => {
  ...
  return fetch(roomLiveInfoUrl)
    ...
    .then((html) => {
      const result = {};
      result.title = html.match(/"introduction":"(\S+?)"/)[1];
      result.snapshotUrl = html.match(/"screenshot":"(\S+?)"/)[1];
      result.username = html.match(/"nick":"(\S+?)"/)[1];
      ...
      result.avatarUrl = html.match(/"avatar180":"(\S+?)"/)[1];
